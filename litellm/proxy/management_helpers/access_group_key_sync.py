...
def _raw_executor(prisma_client: object) -> _RawExecutor:
    ...
    Pinned to the writer: the statements this module runs are UPDATEs issued through
    `query_raw` (the only Prisma raw call that surfaces RETURNING rows), and the
    read-replica routing classifies `query_raw` as a read, so an unpinned executor
    would send them to a read-only replica (#38667).
    """
